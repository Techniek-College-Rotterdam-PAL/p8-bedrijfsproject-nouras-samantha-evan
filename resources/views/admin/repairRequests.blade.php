                    ...
                    @foreach ($repairRequests as $request)
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="py-2 px-4 border-b border-gray-300">
                                <form action="{{ route('admin.updateRepairRequestStatus', $request->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('PUT')
                                ...
                                <form action="{{ route('admin.deleteRepairRequest', $request->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this repair request?');">
                                    @csrf
                                    @method('DELETE')
